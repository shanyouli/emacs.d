...
(defvar my-scratch-root-dir (expand-file-name "scratch-labs/"
                                              (or (bound-and-true-p doom-local-dir)
                                                  user-emacs-directory))
  "my test 语言 scratch 的根目录")

(defvar my-scratch-language-configs
  '((python       :init ("uv" "init" "--lib" "-q") :extension "py" :entry "main.py" :shebang "#!/usr/bin/env python")
    (javascript   :init ("bun" "init" "-y")       :extension "js" :entry "index.js")
...
(defun my-scratch-exec-first-available (commands)
  "查找并执行第一个可用的初始化工具。"
  (let ((cmds (if (stringp (car commands)) (list commands) commands))
        (success nil))
    (cl-loop for cmd in cmds
             when (executable-find (car cmd))
             do (progn
                  (apply #'call-process (car cmd) nil nil (cdr cmd))
                  (setq success t))
             until success)
    success))
...
(defun my-open-scratch-workspace (lang-input)
  ...
  (interactive (list (completing-read "选择语言(输入任意字符进入 zztemp 目录:)"
                                      (mapcar #'car my-scratch-language-configs))))
  (unless (file-exists-p my-scratch-root-dir) (make-directory my-scratch-root-dir t))
  (let* ((lang-sym (intern lang-input))
    ...
    (if is-zztemp
        ...
        (let ((lang-root (expand-file-name "zztemp" my-scratch-root-dir))
          ...
          (unless (file-exists-p lang-root) (make-directory lang-root t))
          (find-file (expand-file-name entry-name lang-root))
          (message (format "open %s" entry-name)))
      (let* ((entry-name (or (plist-get config :entry)
                             (concat "test." (or (plist-get config :extension)
                                                 lang-input))))
            ...
            (when (file-exists-p entry-path)
              (let* ((mtime (nth 5 (file-attributes entry-path)))
                ...
                (unless (string= file-date today)
                  (unless (file-exists-p archive-path) (make-directory archive-path))
                  (rename-file entry-path
                  ...
                  (shell-command (format "git commit -m 'Archive %s'" file-date))
                  (my-scratch-create-new-entry entry-path
                                               (plist-get config :shebang)))))
          (make-directory lang-root t)
          (let ((default-directory lang-root))
            ;; 使用专业工具对项目进行初始化
            (when init-commands (my-scratch-exec-first-available init-commands))
            (unless (file-exists-p (expand-file-name ".git" lang-root))
              (shell-command "git init")
              (with-temp-file ".gitignore"
                (insert "__pycache__/\nnode_modules/\n.venv/\n")))
            (unless (file-exists-p entry-path)
              (my-scratch-create-new-entry entry-path (plist-get config :shebang)))
            ;; 进行第一次 git commit
            (shell-command "git add .")
            (shell-command (format "git commit -m 'First commit on %s'" today))))
