...
;; (cl-defmacro lib-key-repeat (key-prefix &key map &rest args)
...
(defun lib-key--repeat (key-prefix next-key command keymap)
  (lib-key (kbd (concat key-prefix " " next-key))
           (lambda (arg)
             (interactive "p")
             (let ((keymap (make-sparse-keymap)))
               (lib-key next-key command keymap)
               (funcall command arg)
               (set-transient-map keymap t)))
           keymap))
