...
(defun +my-title-set-fn (swith)
  ...
  ;; 从剩余参数列表中弹出下一个值
  (setq my-title (pop command-line-args-left))
  (elog-info my-logger (format "set title prefix: %s" my-title)))

(add-to-list 'command-switch-alist '("--my-title" . +my-title-set-fn))

(setq frame-title-format
      '(:eval (cond ((frame-parameter nil 'my-client)
                     (my-title-stye "EmacsClient"))
                    ((bound-and-true-p my-title)
                     (my-title-stye my-title))
                    (t (my-title-stye "DoomEmacs"))))
      icon-title-format t)
