...
(setopt user-full-name "Shanyou Li"
      ...
      org-agenda-files (my-path-join org-directory "gtd.org")
...
#+begin_src emacs-lisp :noweb-ref none :tangle yes
(after! org-capture
  (cl-defun my-org-agenda-todo-date-tree (&optional time &key return olp)
    "我的TODOS date 树"
    (let ((org-reverse-datetree-level-formats
           '("TODOS"
             ...
             "%Y年%m月"
             "%Y年 第%W周"
             (lambda (time) (let ((year-month-day (format-time-string "%Y年%m月%d日" time))
                             (weekday (pcase (format-time-string "%w" time)
                                        ("0" "星期日")
                                        ("1" "星期一")
                                        ("2" "星期二")
                                        ("3" "星期三")
                                        ...
                                        ("5" "星期五")
                                        ("6" "星期六"))))
                         (format "%s %s" year-month-day weekday))))))
      (org-reverse-datetree-goto-date-in-file time :return return :olp olp)))
  
  (setq org-capture-templates
        (doct `(("TODOS" :keys "t"
                 :file ,(my-path-join org-directory "gtd.org")
                 :function my-org-agenda-todo-date-tree
