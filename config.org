...
** init-script
将编程语言的草稿归档到对应的文件中，目前支持 python，zig，等。
* autoload 库
最终归宿为 lib 中的一个库。一些有用但不确定是否归档的函数合集。
** initfont
用来处理初始化字体的问题, 当默认字体不存在时，加载该库。 ~autoload/initfont~
** 根据输入法的状态配置 cursor[fn:1]
#+begin_src emacs-lisp :tangle "autoload/init-cursor.el"
;;; autoload/init-cursor.el -*- lexical-binding: t; -*-

(defcustom my-ime-active-color "DarkRed"
  "Preferred background color when input method is active."
  :type 'string)
(defcustom my-ime-active-fallback-color "DarkOrange"
  "Second background color when input method is active."
  :type 'string)
(defcustom my-ime-face-near-same-color-threshold
  (or (bound-and-true-p face-near-same-color-threshold) 15000)
  "Determine the smallest parity value for two colors that are similar."
  :type 'int)

(defvar my-theme-original-cursor nil "theme default cursor color.")
(defvar my-theme-computed-ime-cursor nil "Cursor color after activating the input method.")

;;;###autoload
(defun my-refresh-cursor-color-cache (&rest _)
  "获取当前主题光标色彩的默认值和激活输入法后的色彩。"
  (let* ((raw-color (face-attribute 'cursor :background nil t))
         (default-color (if (and (stringp raw-color) (not (string-empty-p raw-color)))
                                       raw-color
                          (frame-parameter nil 'cursor-color)))
         (dist (color-distance default-color my-ime-active-color)))
    (setq my-theme-original-cursor default-color)
    (setq my-theme-computed-ime-cursor (if (< dist my-ime-face-near-same-color-threshold)
                                           my-ime-active-color
                                         my-ime-active-fallback-color))
    ;; DEBUG
    ;; (message "Cursor Cache Updated: Default [%s], IME [%s]" my-theme-original-cursor
    ;;          my-theme-computed-ime-cursor)
    ))
;;;###autoload
(defun my-apply-cursor-style (&optional active)
  "根据输入法状态应用对应的光标颜色。
ACTIVE 为 非-nil 时应用激活输入法后的颜色，反之使用主题默认光标色彩。"
  (unless my-theme-computed-ime-cursor (my-refresh-cursor-color-cache))
  (let ((final-color (if active my-theme-computed-ime-cursor my-theme-original-cursor)))
    (if (and  (bound-and-true-p evil-local-mode)
              (and (boundp evil-state) (not (eq evil-state 'emacs))))
          (progn
            (setq evil-normal-state-cursor (list 'box final-color)
                  evil-visual-state-cursor (list 'hollow final-color)
                  evil-insert-state-cursor (list 'bar final-color))
            (evil-refresh-cursor))
       ;; --- 非 Evil 路径 (原生 Emacs 或 Evil-Emacs-State) ---
        (set-cursor-color final-color))))
#+end_src
- 使用代码
#+begin_src emacs-lisp :tangele "no"
;; 根据主题切换自动更新色彩
(if (boundp 'enable-theme-functions)
    (add-hook 'enable-theme-functions #'my-refresh-cursor-color-cache)
  (advice-add 'load-theme :after #'my-refresh-cursor-color-cache))
;; 2. 根据输入法状态切换钩子
(add-hook 'input-method-activate-hook   (lambda () (my-apply-cursor-style t)))
(add-hook 'input-method-deactivate-hook (lambda () (my-apply-cursor-style nil)))

;; 3. Evil 专用
(with-eval-after-load 'evil
  ;; 插入模式退出时根据输入法状态恢复颜色
  (add-hook 'evil-insert-state-exit-hook
            (lambda () (my-apply-cursor-style current-input-method)))

  ;; 插入模式进入时关闭输入法 (如果需要)
  (add-hook 'evil-insert-state-entry-hook
            (lambda () (when current-input-method (deactivate-input-method))))

  ;; 状态切换时(如切到 Emacs State) 重新校验颜色
  (add-hook 'evil-state-transition-hook
            (lambda () (my-apply-cursor-style current-input-method))))
#+end_src
** 未分类
:PROPERTIES:
...
;;; autoload/init-common.el -*- lexical-binding: t; -*-
<<init-common>>
#+end_src
- 提供我习惯的标题命令风格
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
;;;###autoload
(defun +my--configure-project-root-and-display (arg display-fn)
    "Set the environment variable PROOT and displays a terminal using `display-fn'.
If prefix ARG is non-nil, cd into `default-directory' instead of project root.
Returns the eat buffer. copy form doomemacs:vterm"
    (let* ((project-root (or (doom-project-root) default-directory))
           (default-directory (if arg default-directory project-root)))
      (setenv "PROOT" project-root)
      (funcall display-fn)))
#+end_src
- 文件路径处理相关函数，其主要来源：[fn:2]
#+begin_src emacs-lisp
;;;###autoload
(defun my-path-relative-p (path)
  "Return t if PATH is relative, false otherwise."
  (not (file-name-absolute-p path)))

;;;###autoload
(defun my-path-expand (path &optional dir)
  "Expand PATH relative to DIR, File name handlers are ignored."
  (let (file-name-hander-alist)
...
Be aware if one of the arguments is an absolute path, `my-f-join'
will discard all the preceeding arguments and make this absolute
path the new root of the generated path."
  (let ((relative (my-path-relative-p (car args)))
        path)
    (setq path (expand-file-name (apply #'file-name-concat args)))
    (if relative (file-relative-name path) path)))
#+end_src
- 日期处理
+ 一个更好的当前 date 获取函数[fn:3]
#+begin_src emacs-lisp
(defvar my-n-format "%Y-%m-%dT%H:%M:%S"
  "默认使用的时间格式，具体信息请参考 `format-time-string'")
;;;###autoload
(cl-defun my-n (&key (h 0) (m 0) (s 0) (yy 0) (mm 0) (dd 0))
  "更简洁、高效的时间偏移函数。"
  (let* ((now (decode-time))
         (delta (make-decoded-time :second s :minute m :hour h
                                   :day dd :month mm :year yy))
         (new-time (decoded-time-add now delta)))
    ;; 直接使用 format-time-string 对结果进行格式化，更安全
    (format-time-string my-n-format (encode-time new-time))))
#+end_src
+ 插入当前日期和时间
#+begin_src emacs-lisp
;;;###autoload
(defun insert-datetime ()
  "Insert date at poin."
  (interactive)
  (let ((my-n-format "%Y-%m-%d %H:%M:%S"))
    (insert (my-n))))
#+end_src
- better toggle input method, 参考[fn:4]
#+begin_src emacs-lisp
;;;###autoload
...
(setopt user-full-name "Shanyou Li"
      ...
      my-org-agenda-file (my-path-join org-directory "gtd.org")
      my-workdir (my-path-join "~/Code")
      my-blog-dir (my-path-join my-workdir "Git" "Blog")    ;; My Blog Dir
      my-scratch-root-dir (my-path-join my-workdir "Scratchs"))   ;; 草稿测试文件
#+end_src
...
(let ((lang-dir (my-path-join doom-user-dir "lang" )))
...
(defun load-part-el-file (filename)
  ...
  (if (member filename my-loaded-part-el-list)
      t
    (let ((filepath (my-path-join doom-user-dir filename)))
...
(lib-load "auto-space" my-auto-space-mode my-is-chinese-character global-my-auto-space-mode)
#+end_src
*** 懒加载相关库
- initframe
#+begin_src emacs-lisp
...
- init-scratch
#+begin_src emacs-lisp
(lib-load "scratch" my-open-scratch-workspace my-scratch-set-language-config)
#+end_src
- auto-space
#+begin_src emacs-lisp
(lib-load "auto-space" my-auto-space-mode my-is-chinese-character global-my-auto-space-mode)
...
(add-hook! 'doom-first-buffer-hook (global-subword-mode 1))             ; 识别驼峰，而不是傻瓜前进
      ...
      '("\\.?cache" ".cask" "url" "COMMIT_EDITMSG\\'" "bookmars"
        ...
        (lambda (file)
          (or (file-directory-p file)
              ...
              (file-in-directory-p file (bound-and-true-p persp-save-dir))))))

(after! recentf
  (push (expand-file-name recentf-save-file) recentf-exclude)
  (push (expand-file-name (doom-session-file)) recentf-exclude))
#+end_src
- 自动保存文件
...
- 特定目录下的文件默认只读
#+begin_src emacs-lisp
(defun my-set-readonly-for-specific-path ()
  (let ((bfn (buffer-file-name)))
    (when (or (string-match-p (my-path-join "~" ".emacs.d") bfn)
            (string-match-p (my-path-join "~" ".config" "emacs") bfn)
            (string-match-p (my-path-join (getenv "XDG_CONFIG_HOME") "emacs") bfn))
      (read-only-mode 1))))

(add-hook 'find-file-hook #'my-set-readonly-for-specific-path)
#+end_src
** 日历配置
#+begin_src emacs-lisp
      ...
      (:prefix-map ("d" . "my prjs")
       ...
       :desc "org" "o" (cmd!
                        (+find-file-in-project (or (bound-and-true-p org-directory)
                                                   "~/org")))
       :desc "draft" "t" #'my-open-scratch-workspace
       :desc "py" "y" (cmd! (my-open-scratch-workspace "python"))
...
- 默认标题规则
#+begin_src emacs-lisp
(defun +my-title-set-fn (swith)
  "自定义 Emacs 标题开头的prefix. 推荐： EmacsClient or DoomEmacs"
  ;; 从剩余参数列表中弹出下一个值
  (let ((arg (downcase (pop command-line-args-left))))
    (cond ((string= arg "emacsclient")
           (elog-info my-logger "Set title prefix: EmacsClient")
           (set-frame-parameter nil 'my-client t))
          ((string= arg "doomemacs")
           (elog-info my-logger "Set title prefix: DoomEmacs")
           (set-frame-parameter nil 'my-doom t))
          (t
           (set-frame-parameter nil 'my-doom t)
           (elog-info my-logger
             "set title prefix only support: emacsclient or doomemacs")))))

(add-to-list 'command-switch-alist '("--my-title" . +my-title-set-fn))

(setq frame-title-format
      '(:eval (cond ((frame-parameter nil 'my-doom)
                     (my-title-style "DoomEmacs"))
               ((frame-parameter nil 'my-client)
                (my-title-style "EmacsClient"))
               (t (my-title-style "DoomEmacs"))))
      icon-title-format t)
...
(add-hook 'after-make-frame-functions
          (lambda (frame)
            (let ((frame-name (frame-parameter frame 'name)))
              ...
              (unless (frame-parameter frame 'my-client)
                (elog-info my-logger (format "my-client %S" (frame-parameter frame 'my-client)))
                (elog-info my-logger (format "frame-name %S" frame-name))
                (if (member frame-name '("DoomEmacs" "Emacs" "emacs" "doomemacs"))
                    (set-frame-parameter frame 'my-doom t)
                  (when (and (frame-parameter frame 'client)
                             (< 1 (my-get-all-display-frame)))
...
+ ~doom-font~ : set default font, 我使用 "Pragma Sevka SC" size 为 13,
  或者 "Cascadia Code", size 为 12 ，备选字体: Fantasque Sans Mono, size 为 13
 ...
 (setq
  ...
  '(:long
    ("evil"
     ...
     "buffer-position"
     "buffer-size"
     "flymake" "mise" "major-mode"
     "envrc")
    :short ("evil" "buffer-name" "buffer-position" "flymake" "major-mode"))
...
#+begin_src emacs-lisp
(add-hook! doom-first-buffer-hook #'global-my-auto-space-mode)
(add-hook! insert-translate-name-active-hook
  (when (bound-and-true-p global-my-auto-space-mode)
    (my-auto-space-mode -1)))
(add-hook! insert-translated-name-inactive-hook
           (when (bound-and-true-p global-my-auto-space-mode)
             (my-auto-space-mode +1)))
#+end_src
- 创建 nix 文件时，不导入自动模板
...
(package! emt :recipe (:host github :repo "roife/emt" :files ("*.el" )) :built-in 'prefer)
#+end_src
#+begin_src emacs-lisp
(use-package! emt
  :commands (emt-mode)
  ...
  :init
  (unless (bound-and-true-p emt-lib-path)
    (setq emt-lib-path
          (my-path-join user-emacs-directory
                        "modules"
                        (format "libEMT.%s" (cond (IS-LINUX "so")
                                                  (IS-MAC "dylib")
                                                  (IS-WINDOWS "dll")))))))
#+end_src
** 统计字数
...
** evil
- 变量 my-evil-temp-status-is-edit-p, 用来临时缓存当前 buffer 是否为编辑模式
#+begin_src emacs-lisp
(defvar my-evil-temp-status-is-edit-p nil "Whether the temporary record is currently in edit mode")
#+end_src
* 搜索
** color-rg
...
** Input
基本 toggle-input-method
#+begin_src emacs-lisp
(map! "C-\\" #'my-toggle-input-method)
#+end_src
- 根据输入法是否启用设置不同的光标配色
#+begin_src emacs-lisp
(add-hook! doom-load-theme-hook #'my-refresh-cursor-color-cache)
;; 2. 输入法状态切换钩子
(add-hook 'input-method-activate-hook   (lambda () (my-apply-cursor-style t)))
(add-hook 'input-method-deactivate-hook (lambda () (my-apply-cursor-style nil)))
...
(after! evil
  ;; 插入模式退出时根据输入法状态恢复颜色
  (add-hook! evil-insert-state-exit-hook (my-apply-cursor-style current-input-method))
  ;; 插入模式进入时关闭输入法 (如果需要)
  ;; (add-hook! evil-insert-state-entry-hook (when current-input-method (deactivate-input-method)))
  ;; 状态切换时(如切到 Emacs State) 重新校验颜色
  (add-hook! evil-state-transition-hook (my-apply-cursor-style current-input-method)))
#+end_src
*** Emacs-rime
...
(use-package! rime
  ...
  :bind (("s-." . '+rime-convert-string-at-point)
         :map rime-mode-map ("C-`" . 'rime-send-keybinding))
  :hook (insert-translated-name-active . (lambda ()
                                           (when (and (not (featurep 'rime))
  ...
  ;; FIX: rime 和 eat-mode 的兼容问题. @see: https://github.com/DogLooksGood/emacs-rime/issues/248
  ;; 无法启用。增加以下advice后可以绕过
  (define-advice rime--text-read-only-p (:around (old-fn) allow-eat-mode)
    (if (eq major-mode 'eat-mode)
        nil
      (funcall old-fn)))
  ;; 中文可以正常上屏，但是回车（上屏输入的英文）无法工作。
  ...
  ;; 在eat buffer一行的末尾，输入包含多个中文字符的拼音后，窗口会向左滚动无法恢复。
  ...
  格，以解决 `posframe' 偶尔吃字的问题。"
    (cl-destructuring-bind (content) args
      (let ((newresult (if (or (eq rime-posframe-style 'vertical)
...
*** rimel
一款基于 liberime 的包，比 emacs-rime 更轻量的中文输入法[fn:5], 更多讨论[[https://emacs-china.org/t/liberime-emacs-rime/31225][emacs-china rimel 发布]]
#+begin_src emacs-lisp :tangle "packages.el"
(package! liberime :built-in 'prefer)
(package! rimel :recipe (:host github :repo "jixiuf/rimel"))
#+end_src
#+begin_src emacs-lisp
(use-package! rimel
  :init
  ...
  :hook (insert-translated-name-active . (lambda ()
                                           (when (and (not (featurep 'rimel))
                                                      (string= "rimel" default-input-method))
                                               (require 'rimel nil t))))
  :custom
  (rimel-show-candidate 'posframe) ; 优先使用 posframe
  (rimel-highlight-first t) ; 高亮第一个候选项
  (rimel-posframe-style 'vertical) ; or horizontal
  (rimel-posframe-min-width 15)
...
(when (modulep! :completion vertico)
  (after! vertico
;; (run-with-idle-timer 0.4 nil (lambda () (setq vertico-count 10)))
...
(use-package! lsp-bridge
  ;; :hook (doom-first-buffer-hook . enable-lsp-bridge-for-modes)
...
(map! :leader
  (:prefix "o"
   (:when (fboundp 'eat)
    :desc "Toggle EAT" "s" #'+eat/toggle
    :desc "EAT Here" "S" #'+eat/here)
   (:when (fboundp 'aweshell-toggle)
...
#+end_src
*** eat
在 macos 打开 eat 出现类似 不支持 eat-truecolor 的错误请执行 M+x ~eat-compile-terminfo~[fn:6]
#+begin_src emacs-lisp :tangle "packages.el"
(package! eat :recipe (:type git :host codeberg
                       :repo "akib/emacs-eat"
                       :depth 1
...
(use-package! eat
  ...
  :commands (+eat/toggle +eat/here)
  :hook ((eshell-load . eat-eshell-mode)
         (eshell-load . eat-eshell-visual-command-mode))
  :config
  (elog-info my-logger "starting eat some configure.")
  ;; (after! evil (evil-set-initial-state 'eat-mode 'emacs))
  (set-popup-rules! '(("^\\*eat" :size 0.25 :vslot -4 :select t :quit nil :ttl 0)
                      ("^\\*my:\\(?:v?term\\|e?shell\\|eat\\)-popup"  ; editing buffers (interaction required)
...
Returns the vterm buffer."
    (interactive "P")
    (+my--configure-project-root-and-display
     arg
     (lambda ()
       (let ((buffer-name (format "*my:eat-popup:%s*"
         ...
         (if-let* ((win (get-buffer-window buffer-name)))
             (delete-window win)
           (if-let ((buffer (cl-loop for buf in (doom-buffers-in-mode 'eat-mode)
                                      if (equal (buffer-local-value '+eat--id buf)
                                                buffer-name)
                                      return buf)))
...
(use-package! disable-mouse
  :hook (doom-first-buffer-hook . global-disable-mouse-mode)
  :config
  (after! evil
...
(use-package! dwim-shell-command
  :autoload dwim-shell-command-on-marked-files
  :commands (dwim-shell-command
             my/dwim-shell-commands-unzip
             my/dwim-shell-commands-zip
             my/dwim-rsync-copy-to-dir
             my/dwim-rsync-move-to-dir)
 :init (map! "M-!" #'dwim-shell-command)
 :config
 (defun my/dwim-rsync-copy-to-dir (dest-dir)
   "使用 rsync 将 Dired 中标记的文件复制到 DEST-DIR。"
   (interactive (list (expand-file-name (read-directory-name "DEST DIR: "))))
   (dwim-shell-command-on-marked-files
    "Use rsync to copy files to a specified directory."
    (format "rsync -ahvz --progress --no-i-r '<<f>>' %s"
 ...
 (defun my/dwim-rsync-move-to-dir (dest-dir)
   "使用 rsync 将 Dired 中标记的文件移动到 DEST-DIR。"
   (interactive (list (expand-file-name (read-directory-name "DEST DIR: "))))
   (dwim-shell-command-on-marked-files
    "Use rsync to move files to a specified directory."
    (format "rsync -ahvz --progress --no-i-r --remove-source-files <<f>> %s"
            (shell-quote-argument dest-dir))
    :no-progress t
    :error-autofocus t
    :silent-success t
 ...
 (defun my/dwim-shell-commands-unzip ()
   ...
   (cond
    ((executable-find "atool")
     (dwim-shell-command-on-marked-files
      "Unzip"
      "atool --extract --explain '<<f>>'"
      :utils "atool"))
    ((executable-find "ouch")
     (dwim-shell-command-on-marked-files
      "Unzip"
      "ouch decompress '<<f>>' --dir '<<fne>>'"
      :utils "ouch"))
    (_
     (warn "Please install ouch or atool"))))
 ...
 (defun my/dwim-shell-commands-zip (file)
                  ...
                  (progn
                    (setq dir
                          (concat
                           ...
                           ".tar.gz"))
                    (list
                     (read-string (format "Archive Name(%s):" dir)
                                  nil
                                  nil
                                  dir)))
   ...
   (dwim-shell-command-on-marked-files
    "zip"
    (format "atool -F .tar.xz --add %s '<<*>>'" file)
    :utils "atool")))
#+end_src
** evil
...
(use-package! insert-translated-name
  ...
  :custom (insert-translated-name-translate-engine "deeplx")
  :init
  (defvar insert-translated-name-active-hook nil
    "Input methods to deal with after activation, adding spaces, etc. hook")
  (defvar insert-translated-name-inactive-hook nil
    "Finish processing hook after conversion.")
  :config
  ...
  (defadvice! +insert-translated-name-active (fn &rest args)
    "处理某些可能影响 `insert-translated-name-active' 的行为."
    :before #'insert-translated-name-active
    (doom-run-hooks 'insert-translated-name-active-hook))
  (defadvice! +insert-translated-name-inactive (fn &rest args)
    :after #'insert-translated-name-inactive
    (doom-run-hooks 'insert-translated-name-inactive-hook))

  (add-hook 'insert-translate-name-active-hook
            (lambda ()
              (when (bound-and-true-p evil-mode)
                (setq my-evil-temp-status-is-edit-p
                      (or (evil-insert-state-p)
                          (evil-emacs-state-p)))
                (unless my-evil-temp-status-is-edit-p
                  (evil-insert-state)))))
  (add-hook 'insert-translated-name-inactive-hook
            (lambda ()
              (when (and (bound-and-true-p evil-mode)
                         (not my-evil-temp-status-is-edit-p))
...
** appine
#+begin_quote
Appine 一个允许将 macOS 原生视图直接嵌入 Emacs 窗口的工具，目前支持
webkit, PDFKit, Quick look PreviewView
#+end_quote
#+begin_src emacs-lisp :tangle "packages.el"
(package! appine :recipe (:repo "chaoswork/appine" :host github :depth 1))
#+end_src
#+begin_src emacs-lisp
(use-package! appine
...
** nushell
#+begin_src emacs-lisp :tangle "packages.el" :noweb-ref none
(package! nu-ts-mode :recipe (:host github :repo "ItsOleks/nu-ts-mode"))
...
(use-package! nu-ts-mode
  ...
  (set-tree-sitter! 'nushell-mode 'nu-ts-mode
    `((nu :url "https://github.com/nushell/tree-sitter-nu")))
  :config (+nushell-common-config 'nu-ts-mode))
#+end_src
** TODO python
*** 格式化
**** 格式化工具
...
** other
一些使用 doom 自带配置的，某些修改
*** fennel 使用
使用 fnlfmt 工具进行格式化
#+begin_src emacs-lisp :tangle (if (executable-find "fnlfmt") "yes" "no")
...
(setq org-hugo-base-dir my-blog-dir)
...
(defadvice! +my-org-capture-init-default-h ()
        ...
        (doct
         `(;; --- 1. TODOS (动态日期树) ---
           ("TODOS"
            ...
            :todo-state "TODO"
            :template ("* %{todo-state} [#%^{优先级|B|A|C}] %^{Description}"
                       ":PROPERTIES:"
                       ":Created: %U"
                       ":END:"
                       "%?")
            :function ,(lambda ()
                         ...
                         (let ((org-reverse-datetree-level-formats
                                `("TODOS" "%Y年%m月" "%Y年 第%W周" ,#'my-org-get-week-day-string)))
                           ;; 内部已经处理了 TODOS 标题的创建
           ...
           ("Habit"
            ...
            :file ,my-org-agenda-file
            :todo-state "NEXT"
            ;; 子项会继承 template 和 function
            :template ("* %{todo-state} [#%^{优先级|B|A|C}] %^{习惯名称}"
                       "SCHEDULED: <%t %{hibit}>"
                       ":PROPERTIES:"
                       ":STYLE: habit"
            ...
            :function ,(lambda ()
                         (require 'org-reverse-datetree)
                         (let ((org-reverse-datetree-level-formats (doct-get :my-olp)))
                           ;; 内部已经处理了 TODOS 标题的创建
                           (org-reverse-datetree-goto-date-in-file nil)))
            :children (("每日" :keys "d" :my-olp ("Habit" "Days")   :hibit ".+1d")
                       ("每周" :keys "w" :my-olp ("Habit" "Weeks")  :hibit ".+1w")
                       ("每月" :keys "m" :my-olp ("Habit" "Months") :hibit ".+1m")
                       ("其他" :keys "o" :my-olp ("Habit" "Other")  :hibit "%^{周期|.+1d|.+1w|.+1m}")))
           ...
           ("INBOX"
            ...
            :file ,my-org-agenda-file
            :children (("收集箱" :keys "i" 
                        :headline "INBOX" ; 如果 INBOX 也不存在，建议也改用 :function
                        :template ("* %^{Description}" ":PROPERTIES:" ":Created: %U" ":END:"))
                       ("Protocol"
                        :keys "p"
                        :headline "INBOX"
                        :immediate-finish t
                        :kill-buffer t
                        :template ("* %:annotation %:initial"
                                   ":PROPERTIES:" ":Created: %U" ":END:"))))))))

(defun my-org-get-week-day-string (&optional time)
  "格式化日期层级：2026年01月21日 星期三"
  (let* ((t-val (or time (current-time)))
         (ymd (format-time-string "%Y年%m月%d日" t-val))
         (day-list '("星期日" "星期一" "星期二" "星期三" "星期四" "星期五" "星期六"))
         (wday (nth (string-to-number (format-time-string "%w" t-val)) day-list)))
    (format "%s %s" ymd wday)))
#+end_src
** org-protocol
配置 org-protocol
...
[fn:1] https://emacs-china.org/t/os-smart-input-source/13436/1160

[fn:2] https://github.com/rejeep/f.el

[fn:3]https://paste.sr.ht/~zyd/d6350d65005b6547882548c35923cfddafcfcd65

[fn:4] https://github.com/redguardtoo/emacs.d/blob/3da56451132e10ed5f925aa32adff19185bff705/lisp/init-chinese.el#L8

[fn:5] https://github.com/jixiuf/rimel

[fn:6] https://codeberg.org/akib/emacs-eat/issues/45
