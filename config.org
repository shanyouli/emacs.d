...
#+end_src
- 强制修改所有用 emacsclient 创建的窗口名，初始为 ~EmacsClient~
#+begin_src emacs-lisp
(add-hook 'after-make-frame-functions
          (lambda (frame)
            (when (frame-parameter frame 'client)
              (modify-frame-parameters
               frame '((title . "EmacsClient")))))
          100)
#+end_src
- 更好的标题规则
#+begin_src emacs-lisp
(setq frame-title-format
      '(:eval (let ((client (frame-parameter nil 'client))
                    (prj-name (projectile-project-name)))
                (concat (if client "EmacsClient - " "DoomEmacs - ")
                        "%b"
                        (unless (string= "-" prj-name) (format " - [%s]" prj-name))))))
