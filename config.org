...
在中文字符和英文字符间自动插入空格
#+begin_src emacs-lisp :tangle "autoload/init-auto-space.el"
;; 可以使用零宽字符, "\u200B"
...
一些其他的相同字体大小下等宽且等高的字体有
1. ioskeva font + LXWG WenKai Mono (当放大到一定程度后字体不再等宽)
*** 初始化 frame 大小
#+begin_src emacs-lisp :tangle "autoload/init-frame.el"
;;;###autoload
...
;;;###autoload
(defun my-display-height ()
  "获取当前 display 的 height."
  (nth 4 (assq 'geometry (frame-monitor-attributes))))

;;;###autoload
(defun my/init-default-frame (&optional frame wratio hratio)
  "设置 `frame'的大小"
  (interactive)
  (let* ((current-display-size (assq 'geometry (frame-monitor-attributes)))
         (x-width (or (alist-get 'width default-frame-alist)
                     (truncate (- (* (nth 3 current-display-size)
                                     (or wratio 0.5)) 20))))
        (y-height (or (alist-get 'height default-frame-alist)
                      (truncate (* (nth 4 current-display-size)
                                   (or hratio 0.5))))))
    (set-frame-size (or frame (selected-frame)) x-width y-height (>= x-width 200))))
#+end_src
...
(defun update-emacs-client-title ()
  (let* ((frame (selected-frame))
    ...
    (when (and (string-prefix-p my-emacs-client-frame-name (frame-parameter frame 'name))
               (not (member buf-name (list " *Minibuf-1*"))))
      (modify-frame-parameters
       frame
       (list (cons 'name
                   (concat my-emacs-client-frame-name " - " buf-name)))))))
#+end_src
*** 记录当前光标位置
...
#+end_src
*** treesit
emacs 内部使用的 语法树（tree-sitter）.
#+begin_src emacs-lisp :tangle "autoload/init-treesit.el"
(require 'cl-seq)

;;;###autoload
(defun my-use-treesite-p ()
...
;;;###autoload
(defun my-treesit-language-available-p (lang)
  "Check treesit language available `LANG'."
  (and (my-use-treesite-p) (treesit-language-available-p lang)))

(defun my--treesit-load-language (lang)
  "Check treesit language available and load `LANG'."
  (if (my-treesit-language-available-p lang)
      (progn
        (treesit-parser-create lang)
        (message "load %S treesit success." lang))
    (if (require 'treesit-auto nil t)
        (let ((treesit-language-source-alist (treesit-auto--build-treesit-source-alist)))
          (treesit-auto--prompt-to-install-package lang)
          (if (ignore-errors (treesit-parser-create lang))
              (message "load %S treesit success." lang)
            (message "load %S error. Please %S treesit exists." lang lang)))
      (message "Please manual install %S tressit lib." lang))))

;;;###autoload
(defun my/load-treesit ()
  "Load tressit lib on major-mode."
  (interactive)
  (pcase major-mode
      ('emacs-lisp-mode (my--treesit-load-language 'elisp))
      ('inferior-emacs-lisp-mode (my--treesit-load-language 'elisp))
      ('lisp-interaction-mode (my--treesit-load-language 'elisp))
      (_ (let ((sym_mode (intern (nth 0 (string-split (symbol-name major-mode) "-")))))
           (my--treesit-load-language sym_mode)))))

;;;###autoload
(defun my-treesit-get-recipe (lang)
  (when (require 'treesit-auto nil t)
    (cl-find-if (lambda (x) (eq (treesit-auto-recipe-lang x) lang))
...
*** org headline 移动到指定文件
#+begin_src emacs-lisp :tangle "autoload/init-my-org.el"
;; 需要的包
(require 'outline)
(require 'org-id)
(require 'seq)
...
;; weekly + date
;; month + weekly + ate
;; year + month + weekly + date
(defcustom my-org-groups-level 1 "my org groups level."
  :type 'integer
  :options '(1 2 3 4))

(defcustom my-org-groups-second-type '(daily weekly month year) "my org groups level."
  :type 'list)

(defun my-org-extract-date-from-filename (filename)
  "Extract date from FILENAME if it matches the pattern YYYY_MM_DD.org or YYYY-MM-DD.org.
Return the date in the format YYYY-MM-DD, or current-date if it doesn't match."
  (if (string-match "\\([0-9]\\{4\\}\\)[_-]\\([0-9]\\{2\\}\\)[_-]\\([0-9]\\{2\\}\\)\\.org$" filename)
      (format "%s-%s-%s"
...
(defun my-org-adjust-headline-levels (content &optional level)
  "Adjust headline levels based on `LEVEL'"
  (let ((level-adjust (or level my-org-groups-level)))
    (replace-regexp-in-string "^\\(\\*+\\)" (lambda (m) (concat m (make-string level-adjust ?*))) content)))

(defun my-org-get-group-heading (time &optional level)
  "Get the group heading based on `date' and `level'"
  (pcase level
    ('year (format-time-string "%Y 年" time))
    ('month (format-time-string "%Y 年 %m 月" time))
    ('weekly (format-time-string "%Y 年 %W 周" time))
    (_ (format-time-string "%Y 年 %m 月 %d 日" time))))

;;;###autoload
(defun my-org-get-group-list (date &optional level types)
  "Generate a list based on `DATE'.
LEVEL is interge, TYPES is list"
  (let ((time (date-to-time date))
        (level (or level my-org-groups-level))
        (types (or types my-org-groups-second-type))
        rlist '())
    (dotimes (i level)
      (push (my-org-get-group-heading  time (nth i types)) rlist))
    rlist))


(defsubst my-org--create-headline-tree (path-list)
  "Create  headline tree by `PATH-LIST'."
  (let ((level 1) end notfound)
    (unless (derived-mode-p 'org-mode)
      (error "Target buffer \"%s\" should be in Org mode" (current-buffer)))
    (goto-char (point-min))
    ;; 从 1 级 headline 定位到 n 级 headline
    (dolist (heading path-list)
      (let* ((heading (concat (make-string level ?*) " " heading))
             (re (concat "^" (regexp-quote heading) "$")))
        (if notfound
            (insert "\n" heading)
          (if (re-search-forward re end t)
              (progn
                (goto-char (match-beginning 0)) ;如果找到了 headline 就移动到对应的位置
...
;;;###autoload
(defun my-org-move-headline-to-target (source-file target-file pattern level types)
  "Move `PATTERN' headlines from SOURCE-FILE to TARGET-FILE.
LEVEL Indicates groups."
  (let ((current-date (my-org-extract-date-from-filename
      ...
      (with-current-buffer (find-file-noselect source-file)
        ...
        (while (re-search-forward pattern nil t)
          (let ((start (match-beginning 0))
                oid content end)
            ;; Ensure the headline has an org-id
            (goto-char start)
            (setq oid (org-id-get-create))
            ;; Get end
            (setq end (progn (org-end-of-subtree t t) (point)))
            ;; Copy the headline and its subtree, and adjust the levels of the headlines
            (setq content (my-org-adjust-headline-levels
                           (buffer-substring-no-properties start end)
                           level))

            (with-current-buffer (find-file-noselect target-file)
              ;; (goto-char (point-min))
              (my-org--create-headline-tree (my-org-get-group-list current-date level types))
              ;; Insert the TODO item
              (insert content)
              (save-buffer))
            ;; Insert a link to the new location
            (goto-char start)
            (delete-region start end)
            (insert (format "#+transclude: [[id:%s][%s]] :level 1\n" oid
                            (replace-regexp-in-string "\\*+ TODO " "" (car (split-string content "\n")))))
            (save-buffer)))))
    ))
;;;###autoload
(defun my-org-remove-extra-blank-lines ()
  "Remove extra blank lines in the current Org buffer, keeping at most one."
  (interactive)
  (save-excursion
    (goto-char (point-min))
...
(add-hook 'after-make-frame-functions
          (lambda (frame)
            (pcase (frame-parameter nil 'name)
              ("scratch" (my/init-default-frame frame))
              ("org-protocol" (my/init-default-frame frame 0.618 0.618)))))
#+end_src
** 最近文件的管理
#+begin_src emacs-lisp
...
:END:
使用 sort-tab 时，请不要随意更改字体，如果要更改字体请关闭 sort-tab
#+begin_src emacs-lisp :tangle "packages.el"
...
使用语法树， treesit
- 安装方法
#+begin_src emacs-lisp :tangle (if (and (fboundp 'treesit-available-p) (treesit-available-p)) "packages.el" "no")
(package! treesit-auto :built-in 'prefer)
#+end_src
- treesit 相关配置
#+begin_src emacs-lisp :tangle (if (and (fboundp 'treesit-available-p) (treesit-available-p)) "yes" "no")
...
(use-package! org-roam
  ...
  (setq my-default-org-roam-directory (f-expand (f-join "~/Documents" "logseq"))
        ...
        my-default-org-agenda-file (f-join my-default-org-roam-directory my-default-org-roam-page "summary/gtds.org"))
  ...
  (if my-default-org-agenda-file
      (progn
        (unless (file-exists-p my-default-org-agenda-file)
          (make-empty-file my-default-org-agenda-file t))
        (defun my-move-todo-on-save-h ()
          (let ((filename (buffer-file-name)))
            (when (and filename
                       (derived-mode-p 'org-mode)
                       (string-prefix-p (expand-file-name org-roam-directory) filename)
                       (not (string= my-default-org-agenda-file filename)))
              (my-org-move-headline-to-target filename
                                              my-default-org-agenda-file
                                              "^\\* TODO .*"
                                              4
                                              '(daily weekly month year)))))
        (add-hook 'before-save-hook #'my-move-todo-on-save-h))
...
;;  :hook (after-init . org-roam-ui-mode)
  :init
  ;; Underscore always becomes underline
  ;; https://github.com/org-roam/org-roam-ui/issues/261
...
#+end_src
#+begin_src emacs-lisp
(after! fennel-mode (add-hook! fennel-mode #'lsp! 'append))
(when (modulep! :tools lsp +eglot)
  (after! eglot
...
#+begin_src emacs-lisp :tangle "packages.el" :noweb-ref none
(package! jsonian :recipe (:host github :repo "iwahbe/jsonian") :built-in 'prefer)
#+end_src

#+begin_src emacs-lisp :tangle "yes" :noweb-ref none
(let ((use-json-treesit-p (my-treesit-language-available-p 'json)))
  (use-package! jsonian
    :defer 0.5
    :init
    (add-hook 'jsonian-mode-hook #'lsp! 'append)
    (add-to-list 'auto-mode-alist '("\\.js\\(?:on\\|[hl]int\\(?:rc\\)?\\)\\'" . jsonian-mode))
    (unless use-json-treesit-p
      (after! org-src (add-to-list 'org-src-lang-modes '("json" . jsonian))))
    :config
  ...
  (when use-json-treesit-p
    (use-package! json-ts-mode
    :defer t
    :init
    (after! treesit-auto
      (let*
          ((old-json-recipe (my-treesit-get-recipe 'json))
           (old-json-remap (treesit-auto-recipe-remap old-json-recipe))
           (json-remap
            (if (listp old-json-remap) (push 'jsonian-mode old-json-remap)
              (list 'jsonian-mode old-json-remap))))
        (add-to-list 'treesit-auto-recipe-list
                     (make-treesit-auto-recipe
                      :lang 'json
                      :ts-mode 'json-ts-mode
                      :url (treesit-auto-recipe-url old-json-recipe)
                      :remap json-remap
                      :revision (treesit-auto-recipe-revision old-json-recipe)
                      :source-dir (treesit-auto-recipe-source-dir old-json-recipe)
                      :ext "\\.js\\(?:on\\|[hl]int\\(?:rc\\)?\\)\\'"))))
    :config
    (after! jsonian (setq json-ts-mode-hook jsonian-mode-hook))
    (after! org-src (add-to-list 'org-src-lang-modes '("json" . jsonian))))))
#+end_src
** sh
*** org mode 中使用代码块
...
(let ((use-nushell-treesit-p (my-treesit-language-available-p 'nu)))
  (use-package! nushell-mode
    :mode "\\.nu\\'"
    :init
    (unless use-nushell-treesit-p
      (after! org-src (add-to-list 'org-src-lang-modes '("nu" . nushell))))
    (after! eglot (add-to-list 'eglot-server-programs '(nushell-mode . ("nu" "--lsp")))))
  (when use-nushell-treesit-p
    (use-package! nushell-ts-mode
      :init
      (after! eglot (add-to-list 'eglot-server-programs '(nushell-ts-mode . ("nu" "--lsp"))))
      (add-hook! nushell-ts-mode #'lsp!)
      (after! org-src (add-to-list 'org-src-lang-modes '("nu" . nushell-ts)))))
  (add-hook! '(nushell-ts-mode-hook nushell-mode-hook) #'lsp!))
#+end_src
...
(use-package! rustic
  ...
  (when (my-treesit-language-available-p 'rust)
...
(my/leader :keymaps 'conf-toml-mode-map
  "f" '(:ignore t :which-key "file")
  "f p" '(+toml-format-region-or-buffer :wk "format"))
(map! :map conf-toml-mode-map "C-M-\\" :desc "indent" #'+toml-format-region-or-buffer)
#+end_src
*** 结尾
#+begin_src emacs-lisp
  )
