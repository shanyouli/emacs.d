...
(defun md-key/unset+ (keymap key)
  "Unset the `key' binding. the keymap is `keymap'."
  (md-key/set+ keymap key nil))
...
(defun md-key/unset-global+ (&rest keybind)
  ...
  (let ((keymap (current-global-map)))
    (md-key/unset-local+ keymap keybind)))



;;;###autoload
(defun md-key/set-ext (key-str def &optional keymap key-prefix filename)
  "函数绑定快捷按键. 用法:
(md-key/set-ext KEY-STR DEF &OPTIONAL KEYMAP KEY-PREFIX FILENAME)
...
`filename' 为当对应函数为被加载时, 确定函数对应的文件,并加载它."
  (let* ((key-prefix (cond ((and md-key-prefix key-prefix (booleanp key-prefix))
         ...
         (keymap (or keymap (current-global-map))))
    ...
    (let (key def)
      (dolist (element key-alist)
        (setq key (car element)
              def (cdr element))
        (md-key/set-ext key def keymap key-prefix filename)))))

;;;###autoload
(defun md-key/set-global (key-alist &rest rest)
  "默认 KEYMAP 为 (current-global-map) 的 `md-key/set-local' 扩展函数."
  (let ((keymap (current-global-map)))
    (md-key/set-local key-alist keymap rest)))
